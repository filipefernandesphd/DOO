# Checklist para correção

### *Usuário*

- [ ] As classes estão em `package br.com.sistema.usuarios`;
- [ ] `Aluno` e `Professor` são subclasses de `Usuario`;
- [ ] `Usuario`, `Aluno` e `Professor` estão como `public`;
- [ ] `Usuario` tem somente um construtor e o método `public T getIdentificacao()`;
- [ ] `Usuario` é uma classe genérica;

### *Questões*

- [ ] As classes estão em `br/com/sistema/provas/questao`;
- [ ] Foi implementado o padrão **Composite**;
- [ ] As classes estão como `public`;
- [ ] `Questao` implementou uma classe interna não estática pública;
- [ ] `public void adicionarRespostaAberta(String repostaaberta)` foi implementado corretamente;
- [ ] `public void adicionarVerdadeiroFalso(boolean afirmacaocorreta)` foi implementado corretamente;
- [ ] `public void adicionarMultiplaEscolha(String descricao)` foi implementado corretamente;
- [ ] `public void adicionarMultiplaEscolha(String descricao, boolean correta)` foi implementado corretamente;


### *Conteúdos Extras*

- [ ] As classes estão em `br/com/sistema/provas/questao/infoadicional`;
- [ ] Foi implementado o padrão **Decorator**;
- [ ] `ConteudoExtra`, `TextoDecorator`, `ImagemDecorator` e `VideoDecorator` estão como `public`;
- [ ] O implementação da classe `TextoDecorator` está correta;
- [ ] O implementação da classe `ImagemDecorator` está correta;
- [ ] O implementação da classe `VideoDecorator` está correta;

### *Prova*

- [ ] As classes estão em `br/com/sistema/provas`;
- [ ] O método `public void adicionarQuestao(BlocoQuestao questao)` de `Prova` foi implementado corretamente;
- [ ] O método `public ArrayList<BlocoQuestao> retornarQuestoes()` de `Prova` foi implementado corretamente;
- [ ] `GeraProva` foi implementada corretamente.
